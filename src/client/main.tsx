...
function App() {
  ...
  useEffect(() => { if (signedIn) refresh().catch(error => setNotice(error.message)); }, [signedIn, activeAgentId]);
  ...
  async function send(event: React.FormEvent) {
    ...
    try {
      ...
      const text = error instanceof Error ? error.message : "Something failed.";
      setNotice(text);
  ...
  return <div className={`app-shell ${toolsOpen ? "tools-open" : "tools-closed"}`}>
    <aside className="sidebar"><div className="brand"><h1>AI Agents</h1><p>Multi-agent workspace</p><button className="new-chat" onClick={() => { setConversationId(undefined); setMessages([]); }}>+ New chat</button></div><div className="sidebar-section history-heading">All Bot History <span>{conversations.length}</span></div><div className="history-list history-list-priority">{conversations.length ? conversations.map(item => <button className={item.id === conversationId ? "history-item active" : "history-item"} key={item.id} onClick={() => loadConversation(item.id).catch(error => setNotice(error.message))}><strong>{item.title}</strong><span>{agentMap.get(item.agent_id) ?? item.agent_id} · {new Date(item.updated_at).toLocaleString()}</span></button>) : <p className="history-empty">No saved chats yet. Send a message and it will appear here.</p>}</div><div className="sidebar-section">Agents</div><nav>{agents.map(agent => <button className={agent.id === activeAgentId ? "agent active" : "agent"} key={agent.id} onClick={() => { setActiveAgentId(agent.id); setConversationId(undefined); setMessages([]); }}><strong>{agent.name}</strong><span>{agent.role}</span></button>)}</nav></aside>
