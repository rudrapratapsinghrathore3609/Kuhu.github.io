...
const defaultProviders = [
  ...
  { provider: "anthropic", baseUrl: "https://api.anthropic.com/v1", model: "claude-3-5-haiku-latest" },
...
const websitePrompts = [
  ...
  "Give me a deployment checklist for Vercel, Netlify, Supabase, env vars, domains, and security."
];
...
type GitIssue = { id: string; title: string; status: "planned" | "doing" | "done"; createdAt: string };
...
function parseSourceCards(text: string): SourceCard[] {
  ...
  return text.slice(start + marker.length).split("\n")
    ...
    .map(line => {
      ...
      const displayLabel = label.trim() || "Source";
      const low = clean.toLowerCase();
      const trust = low.includes("error") || low.includes("none enabled") || low.includes("not used") ? "low"
        : low.includes("stored") || low.includes("saved") || low.includes("used") || low.includes("matched") ? "high"
        : low.includes("available") || low.includes("requested") ? "medium"
        : "unknown";
      const links = Array.from(value.matchAll(/https?:\/\/[^\s|;)]+/g)).map(match => match[0]);
      return { label: displayLabel, value, trust, links };
    });
}
...
function buildSourceSummary(cards: SourceCard[]) {
  ...
  const council = cards.find(card => card.label === "Agent council used")?.value;
  const linkCount = cards.reduce((total, card) => total + card.links.length, 0);
  const parts = [account && `AI: ${account}`, connectors && !connectors.includes("none enabled") && "connectors checked", council && council !== "none" && `team: ${council}`].filter(Boolean);
  return `What happened: ${parts.length ? parts.join(" -> ") : "direct agent response"}${linkCount ? ` -> ${linkCount} source link(s)` : ""}`;
}

function friendlyError(error: unknown) {
  const raw = error instanceof Error ? error.message : String(error || "Something failed.");
  if (/quota|429|billing/i.test(raw)) return "AI account failed: quota or billing limit reached. Try Ollama or another account.";
  if (/fetch|network|failed to fetch/i.test(raw)) return "Connection failed: check that the backend is running and the selected account/connector is reachable.";
  if (/api key|unauthorized|401|403/i.test(raw)) return "Authentication failed: check the API key or connector credentials.";
  if (/MAX_TOKENS/i.test(raw)) return "The model stopped early because the answer was too long. Ask it to continue or switch to Ollama/OpenAI.";
...
function App() {
  ...
  const [teamAgentIds, setTeamAgentIds] = useState<string[]>(() => JSON.parse(localStorage.getItem("kuhuTeamAgents") || "[\"nova\",\"kuhu\"]"));
  ...
  const [tasks, setTasks] = useState<Task[]>(() => JSON.parse(localStorage.getItem("kuhuTasks") || "[]"));
  const [folders, setFolders] = useState<string[]>(() => JSON.parse(localStorage.getItem("kuhuFolders") || JSON.stringify(initialFolders)));
  const [activeFolder, setActiveFolder] = useState("");
  const [newFolder, setNewFolder] = useState("");
  const [accountForm, setAccountForm] = useState({ label: "", provider: "openai", baseUrl: "https://api.openai.com/v1", model: "gpt-4.1-mini", apiKey: "" });
  const [connectorForm, setConnectorForm] = useState({ label: "Memory Search", type: "memory_search", config: "{}" });
  const composerRef = useRef<HTMLFormElement | null>(null);
  const chatRef = useRef<HTMLElement | null>(null);
  const [agentInstructions, setAgentInstructions] = useState<Record<string, string>>(() => JSON.parse(localStorage.getItem("kuhuAgentInstructions") || "{}"));
  const [shareEmail, setShareEmail] = useState("");
  const [githubRepo, setGithubRepo] = useState(() => localStorage.getItem("kuhuGithubRepo") || "");
  const [githubIssueTitle, setGithubIssueTitle] = useState("");
  const [githubIssues, setGithubIssues] = useState<GitIssue[]>(() => JSON.parse(localStorage.getItem("kuhuGithubIssues") || "[]"));
  ...
  const visibleConversations = useMemo(() => {
    ...
    return conversations.filter(item => {
      ...
      const searchable = `${item.title} ${agentName} ${new Date(item.updated_at).toLocaleString()}`.toLowerCase();
      return matchesAgent && (!query || searchable.includes(query));
    });
  ...
  useEffect(() => { if (signedIn) refreshAppData().catch(error => setNotice(error.message)); }, [signedIn, activeAgentId]);
  useEffect(() => localStorage.setItem("kuhuTasks", JSON.stringify(tasks)), [tasks]);
  useEffect(() => localStorage.setItem("kuhuFolders", JSON.stringify(folders)), [folders]);
  useEffect(() => localStorage.setItem("kuhuTeamAgents", JSON.stringify(teamAgentIds)), [teamAgentIds]);
  useEffect(() => localStorage.setItem("kuhuAgentInstructions", JSON.stringify(agentInstructions)), [agentInstructions]);
  useEffect(() => localStorage.setItem("kuhuGithubRepo", githubRepo), [githubRepo]);
  useEffect(() => localStorage.setItem("kuhuGithubIssues", JSON.stringify(githubIssues)), [githubIssues]);
  useEffect(() => {
  ...
  async function refreshAppData() {
    ...
    if (failures.length) setNotice(failures.map(item => item.reason instanceof Error ? item.reason.message : String(item.reason)).join("\n"));
  }
  ...
  async function loadConversation(id: string) {
    ...
    setMessages(data.messages.filter(item => item.role === "user" || item.role === "assistant").map(item => ({ role: item.role as "user" | "assistant", content: contentToText(item.content), id: item.id })));
  }
  ...
  async function authenticate(event: React.FormEvent) {
    ...
    setAuthBusy(false);
    setNotice(error ? error.message : mode === "signup" ? "Account created. Check your email if confirmation is enabled." : "Signed in.");
  }
  ...
  async function testAIAccount(account: Account) {
    ...
    try {
      ...
      setTestResults(current => ({ ...current, [`account:${account.id}`]: error instanceof Error ? error.message : "Account test failed" }));
    }
  }

  async function testSavedConnector(connector: Connector) {
    ...
    try {
      ...
      setTestResults(current => ({ ...current, [`connector:${connector.id}`]: error instanceof Error ? error.message : "Connector test failed" }));
    }
  }
  function routedAccountId(userText: string) {
    if (activeAccountId !== "auto") return activeAccountId;
    const lower = userText.toLowerCase();
    const ollama = accounts.find(item => item.provider === "ollama" || item.base_url.includes("11434"));
    const gemini = accounts.find(item => item.provider === "gemini");
    const openai = accounts.find(item => item.provider === "openai" || item.provider === "compatible");
    if (gemini && /news|source|research|history|current|fact|search|latest|update/.test(lower)) return gemini.id;
    if (ollama) return ollama.id;
    return (gemini ?? openai ?? accounts[0])?.id || "";
  }
  ...
  async function send(event: React.FormEvent) {
    ...
    try {
      ...
      const instructionPrefix = agentInstructions[activeAgentId]?.trim() ? `[Custom agent instructions for ${activeAgent?.name ?? activeAgentId}: ${agentInstructions[activeAgentId].trim()}]\n` : "";
      const userText = instructionPrefix + folderPrefix + (message.trim() || `Please analyze ${uploads.length} uploaded file(s).`);
      const chosenAccountId = routedAccountId(userText);
      if (!chosenAccountId) throw new Error("Add an AI account first.");
      ...
      const done = await streamChat({
        ...
        conversationId,
        message: userText,
        uploadIds: uploads.map(file => file.id),
        advisorAccountIds,
      ...
      setMessages(current => {
        const copy = [...current];
        const last = copy[copy.length - 1];
        if (last?.role === "assistant" && !last.content) copy[copy.length - 1] = { ...last, content: `Error: ${messageText}` };
        return copy;
      });
    } finally {
      setStreaming(false);
  ...
  function startNewChat() { setConversationId(undefined); setMessages([]); setMessage(""); }
  function toggleAdvisor(id: string) { setAdvisorAccountIds(current => current.includes(id) ? current.filter(item => item !== id) : [...current, id]); }
  function toggleTeamAgent(id: string) { setTeamAgentIds(current => current.includes(id) ? current.filter(item => item !== id) : [...current, id]); }
  ...
  function startVoiceCommand() {
    ...
    recognition.onresult = event => {
      ...
      if (transcript) {
        setMessage([baseMessage, transcript].filter(Boolean).join(" "));
      }
    };
    ...
    recognition.onend = () => {
      setListening(false);
      setVoiceStatus(autoSendVoice ? "Voice captured. Sending..." : "Voice captured. You can edit it, then send.");
      if (autoSendVoice) setTimeout(() => composerRef.current?.requestSubmit(), 120);
    };
    recognition.start();
  }

  function speakLatestAnswer() {
    const last = [...messages].reverse().find(item => item.role === "assistant" && item.content.trim());
    if (!last || !("speechSynthesis" in window)) return;
    window.speechSynthesis.cancel();
    const utterance = new SpeechSynthesisUtterance(visibleAnswer(last.content));
    utterance.lang = voiceLang;
    utterance.rate = 0.96;
    window.speechSynthesis.speak(utterance);
  }
  function addTask(title: string) { if (title.trim()) setTasks(current => [{ id: crypto.randomUUID(), title: title.trim(), status: "todo", createdAt: new Date().toISOString() }, ...current]); }
  ...
  function copyShareInvite() {
    const text = `I am inviting you to use my AI Agents app.\n\nLocal link if running on my computer/network: http://localhost:5173\n\nFor online deployment, I will add you as a user/collaborator and share the deployed URL. Suggested invite email: ${shareEmail || "your email"}.`;
    navigator.clipboard?.writeText(text).catch(() => undefined);
    setNotice(text);
  }

  function applyWebSearchTemplate() {
    setConnectorForm({ label: "Web Search", type: "web_search", config: '{"provider":"brave","apiKey":"PASTE_BRAVE_KEY"}' });
  }

  function runTaskPrompt(task: Task) {
    insertPrompt(`Execute this task as a careful workflow. Break it into steps, check dependencies, mention risks, and ask before any destructive action.\n\nTask: ${task.title}`, "automate");
  }

  function addGithubIssueDraft() {
    if (!githubIssueTitle.trim()) return;
    setGithubIssues(current => [{ id: crypto.randomUUID(), title: githubIssueTitle.trim(), status: "planned", createdAt: new Date().toISOString() }, ...current]);
    setGithubIssueTitle("");
  }

  function cycleGithubIssue(id: string) {
    setGithubIssues(current => current.map(issue => issue.id === id
      ? { ...issue, status: issue.status === "planned" ? "doing" : issue.status === "doing" ? "done" : "planned" }
      : issue
    ));
  }

  function sendIssueToCoder(issue: GitIssue) {
    insertPrompt(`Use the Coder agent to plan this GitHub issue for the AI Agents app.\n\nRepository: ${githubRepo || "not set yet"}\nIssue: ${issue.title}\nStatus: ${issue.status}\n\nCreate a short implementation plan, likely files to inspect, safety risks, and verification steps. Do not make destructive changes without approval.`, "coder");
  }
    ...
    return (
      <main className="auth-page">
        <form className="auth-card auth-card-wide" onSubmit={authenticate}>
          ...
          <h1>{mode === "signin" ? "Welcome back" : "Create your command centre"}</h1>
          <p>Sign in once. Your agents, history, memory, uploads, connectors, and settings stay synced with Supabase.</p>
          ...
          <label>Password<input value={password} onChange={event => setPassword(event.target.value)} type="password" autoComplete={mode === "signin" ? "current-password" : "new-password"} required /></label>
    ...
    <div className={`app-shell ${rightPanelOpen ? "tools-open" : "tools-closed"}`}>
      <aside className="sidebar">
        <div className="brand">
          ...
          <button type="button" className="new-chat" onClick={startNewChat}>+ New chat</button>
        </div>
        <div className="sidebar-section history-heading">All Bot History <span>{visibleConversations.length}</span></div>
        <div className="history-controls"><input value={historySearch} onChange={event => setHistorySearch(event.target.value)} placeholder="Search history..." /><select value={historyAgentFilter} onChange={event => setHistoryAgentFilter(event.target.value)}><option value="all">All agents</option>{agents.map(agent => <option key={agent.id} value={agent.id}>{agent.name}</option>)}</select></div>
        <div className="history-list history-list-priority">{visibleConversations.length ? visibleConversations.map(item => <button type="button" className={item.id === conversationId ? "history-item active" : "history-item"} key={item.id} onClick={() => loadConversation(item.id).catch(error => setNotice(error.message))}><strong>{item.title}</strong><span>{agentNameById.get(item.agent_id) ?? item.agent_id} � {new Date(item.updated_at).toLocaleString()}</span></button>) : <p className="history-empty">No saved chats yet. Send a message and it will appear here.</p>}</div>
      ...
      <main className="workspace">
        <header className="topbar">
          ...
          <div className="top-actions">
            ...
            <select value={activeFolder} onChange={event => setActiveFolder(event.target.value)}><option value="">No folder</option>{folders.map(folder => <option key={folder} value={folder}>{folder}</option>)}</select>
        ...
        <section ref={chatRef} className="chat" aria-live="polite">
          ...
          {messages.map((item, index) => {
            ...
            return <article className={`bubble ${item.role}`} key={`${item.id ?? index}-${index}`}>
              ...
              <p>{item.role === "assistant" ? visibleAnswer(item.content) || (streaming ? "Thinking..." : "") : item.content}</p>{item.role === "assistant" && streaming && !item.content && <div className="typing-dots"><span></span><span></span><span></span></div>}
              {cards.length > 0 && <div className="source-summary">{buildSourceSummary(cards)}</div>}
        ...
        <form ref={composerRef} className="composer" onSubmit={send}>
          ...
          <div className="composer-actions"><button type="button" onClick={startVoiceCommand}>{listening ? "Listening..." : "Voice"}</button><button type="button" onClick={speakLatestAnswer}>Speak answer</button><button disabled={streaming || (!activeAccount && activeAccountId !== "auto")}>{streaming ? "Working..." : "Send"}</button></div>
      ...
      {rightPanelOpen && <aside className="right-panel" aria-label="Tools drawer"><button type="button" className="drawer-close" onClick={() => setRightPanelOpen(false)}>Close tools</button>
        ...
        <section className="panel-card"><h2>Agent Studio</h2><p className="panel-help">Add custom instructions for the selected agent. These are included in future prompts.</p><textarea className="small-textarea" value={agentInstructions[activeAgentId] ?? ""} onChange={event => setAgentInstructions(current => ({ ...current, [activeAgentId]: event.target.value }))} placeholder="Example: Be concise, ask before changing code, prefer checklists..." /><button type="button" onClick={() => setAgentInstructions(current => ({ ...current, [activeAgentId]: "" }))}>Clear agent instructions</button></section>

        <section className="panel-card"><h2>Share Workspace</h2><p className="panel-help">Prepare an invite for your friend. For true shared editing, deploy online and add them as a GitHub/Supabase collaborator.</p><div className="search-row"><input value={shareEmail} onChange={event => setShareEmail(event.target.value)} placeholder="Friend email" /><button type="button" onClick={copyShareInvite}>Copy invite</button></div></section>

        <section className="panel-card github-panel"><h2>GitHub Project</h2><p className="panel-help">Track app improvements and hand them to Coder. Live GitHub issue sync can be wired once the connector exposes your account.</p><div className="account-form"><input value={githubRepo} onChange={event => setGithubRepo(event.target.value)} placeholder="Repository URL, e.g. https://github.com/name/repo" /><div className="search-row"><input value={githubIssueTitle} onChange={event => setGithubIssueTitle(event.target.value)} placeholder="New issue or improvement" /><button type="button" onClick={addGithubIssueDraft}>Add issue</button></div></div><div className="github-issue-list">{githubIssues.length ? githubIssues.map(issue => <div className={`github-issue ${issue.status}`} key={issue.id}><button type="button" onClick={() => cycleGithubIssue(issue.id)}>{issue.status}</button><span>{issue.title}</span><button type="button" onClick={() => sendIssueToCoder(issue)}>Coder</button></div>) : <p className="panel-help">No issue drafts yet.</p>}</div><div className="pr-checklist"><strong>Coder PR checklist</strong><span>Plan, branch, patch, lint/test, review, deploy.</span></div></section>

        <section className="panel-card"><h2>Voice Settings</h2><div className="search-row"><select value={voiceLang} onChange={event => setVoiceLang(event.target.value)}><option value="en-IN">English India</option><option value="en-US">English US</option><option value="hi-IN">Hindi India</option></select><label className="advisor-option"><input type="checkbox" checked={autoSendVoice} onChange={() => setAutoSendVoice(value => !value)} /><span>Auto-send after voice</span></label></div></section>

        <section className="panel-card"><h2>Deployment Check</h2><button type="button" onClick={() => runDeployCheck().catch(error => setNotice(error.message))}>Check deployment readiness</button><div className="connector-list">{deployChecks.map(check => <div className={`connector-pill ${check.ok ? "ok" : "bad"}`} key={check.name}><strong>{check.ok ? "Ready" : "Fix"}: {check.name}</strong><span>{check.detail}</span></div>)}</div></section>

        <section className="panel-card"><h2>File Library</h2><p className="panel-help">Recent uploaded files across your chats.</p><div className="connector-list">{fileLibrary.length ? fileLibrary.map(file => <div className="connector-pill" key={file.id}><strong>{file.file_name}</strong><span>{file.mime_type} � {Math.round(file.byte_size / 1024)} KB{file.created_at ? ` � ${new Date(file.created_at).toLocaleString()}` : ""}</span></div>) : <p className="panel-help">No uploaded files yet.</p>}</div></section>

        <section className="panel-card"><h2>Agent Team</h2><p className="panel-help">Turn this on only when you want multiple agents to privately advise the answer. Normal chat stays faster with it off.</p><label className="advisor-option"><input type="checkbox" checked={agentTeamEnabled} onChange={() => setAgentTeamEnabled(enabled => !enabled)} /><span>{agentTeamEnabled ? "Agent Team on" : "Agent Team off"}</span></label>{agentTeamEnabled && <div className="advisor-list">{agents.filter(agent => agent.id !== activeAgentId).map(agent => <label className="advisor-option" key={agent.id}><input type="checkbox" checked={teamAgentIds.includes(agent.id)} onChange={() => toggleTeamAgent(agent.id)} /><span>{agent.name} - {agent.role}</span></label>)}</div>}</section>

        <section className="panel-card"><h2>Model Advisors</h2><p className="panel-help">Let your selected agent consult extra models before answering.</p><div className="advisor-list">{accounts.length ? accounts.map(account => <label className="advisor-option" key={account.id}><input type="checkbox" checked={advisorAccountIds.includes(account.id)} disabled={account.id === activeAccountId} onChange={() => toggleAdvisor(account.id)} /><span>{account.label} - {account.model}{account.id === activeAccountId ? " (main)" : ""}</span></label>) : <p className="panel-help">Add OpenAI, Gemini, Ollama, or Claude first.</p>}</div></section>

        <section className="panel-card"><h2>Web Search Citations</h2><p className="panel-help">Use Brave or Serper so answers can include clickable source links.</p><button type="button" onClick={applyWebSearchTemplate}>Use web search template</button></section>

        <section className="panel-card"><h2>Connectors</h2><form onSubmit={saveConnector} className="account-form"><input placeholder="Connector label" value={connectorForm.label} onChange={event => setConnectorForm({ ...connectorForm, label: event.target.value })} required /><select value={connectorForm.type} onChange={event => setConnectorForm({ ...connectorForm, type: event.target.value })}><option value="memory_search">memory_search</option><option value="web_search">web_search</option><option value="google_drive">google_drive</option><option value="local_files">local_files</option><option value="custom_api">custom_api</option></select><textarea className="small-textarea" value={connectorForm.config} onChange={event => setConnectorForm({ ...connectorForm, config: event.target.value })} /><button>Save connector</button></form><div className="connector-list">{connectors.length ? connectors.map(connector => <div className="connector-pill testable" key={connector.id}><strong>{connector.label}</strong><span>{connector.type} {connector.enabled ? "on" : "off"}</span><button type="button" onClick={() => testSavedConnector(connector)}>Test connector</button>{testResults[`connector:${connector.id}`] && <small>{testResults[`connector:${connector.id}`]}</small>}</div>) : <p className="panel-help">Add Memory Search first. For Web Search, use JSON like {`{"provider":"brave","apiKey":"..."}`} .</p>}</div></section>

        <section className="panel-card"><h2>What AI Knows</h2><p className="panel-help">Editable learned patterns for this agent. Delete anything that feels wrong.</p><div className="memory-list">{memories.length ? memories.map(memory => <div className="memory-item" key={memory.id}><strong>{memory.category}</strong><span>{memory.learning}</span><small>{Math.round(Number(memory.confidence) * 100)}% confidence</small><div><button type="button" onClick={() => editMemory(memory)}>Edit</button><button type="button" onClick={() => removeMemory(memory)}>Delete</button></div></div>) : <p className="panel-help">No learned memories yet for this agent.</p>}</div></section>

        <section className="panel-card"><h2>Automation</h2><form className="search-row" onSubmit={event => { event.preventDefault(); const input = event.currentTarget.elements.namedItem("task") as HTMLInputElement; addTask(input.value); input.value = ""; }}><input name="task" placeholder="Add task or workflow" /><button>Add task</button></form><div className="task-list">{tasks.map(task => <div className={`task-row task ${task.status}`} key={task.id}><button type="button" onClick={() => setTasks(current => current.map(item => item.id === task.id ? { ...item, status: item.status === "todo" ? "doing" : item.status === "doing" ? "done" : "todo" } : item))}>{task.status}: {task.title}</button><button type="button" onClick={() => runTaskPrompt(task)}>Run</button></div>)}</div><button type="button" onClick={() => insertPrompt("Turn my saved tasks into an automation plan with steps, triggers, tools, risks, and confirmation points.", "automate")}>Ask Automate</button></section>
        ...
        <section className="panel-card"><h2>Search Memory</h2><div className="search-row"><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search learned memory..." /><button onClick={runSearch}>Search</button></div>{notice && <pre className="notice">{notice}</pre>}</section>
      </aside>}
      <nav className="mobile-tabbar" aria-label="Mobile navigation"><button type="button" onClick={startNewChat}>Chat</button><button type="button" onClick={() => document.querySelector(".sidebar")?.scrollIntoView({ behavior: "smooth" })}>Agents</button><button type="button" onClick={() => document.querySelector(".history-heading")?.scrollIntoView({ behavior: "smooth" })}>History</button><button type="button" onClick={() => setRightPanelOpen(true)}>Tools</button></nav>
    </div>
  );
}
